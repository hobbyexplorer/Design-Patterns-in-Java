...
import gamemaker.actionform.MoveUpActionFillOutForm;
...
public class MoveUpActionFillOutFormController implements ActionFillOutFormController, ActionListener
{
	...
	public MoveUpActionFillOutFormController(ActionFormsMasterController actionFormsMasterController)
	{
		this.actionFormsMasterController = actionFormsMasterController;
		fillOutForm = new MoveUpActionFillOutForm();
		fillOutForm.AddActionListenerToButtons(this);
		GameMaker.logger.logInfo(this.getClass().getName() + " - Move Up Action Fillout form controller called.");
	...
	public void addAction(){
		...
		rotateImageString = ((MoveUpActionFillOutForm) this.fillOutForm).getRotateSprite().getSelectedItem().toString();
		if(rotateImageString.equalsIgnoreCase("yes"))
			rotateImage = true;
		
		actionToBeAdded = new MoveUp(spriteToAdd, rotateImage);
