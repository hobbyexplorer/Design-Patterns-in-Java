...
public class MoveUp implements Action
{
	...
	public MoveUp(Sprite sprite, Boolean rotateImage)
	{
		this.actionName = Constants.ACTION_MOVE_UP;
		this.sprite = sprite;
	...
	public void execute(GameObject gameObject)
	{
		...
		if(this.rotateImage){
			if(gameObject.getAngle() == 90){
				gameObject.moveObject();
			}
			else{
				int rotate = 90 - gameObject.getAngle();
				gameObject.updateAngle(rotate);
				gameObject.updateImageRotationAngle(rotate);
				gameObject.moveObject();
			}
		}
		else gameObject.moveUp();
	}
