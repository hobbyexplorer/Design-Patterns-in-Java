...
public class MoveDown implements Action
{
	...
	public MoveDown(Sprite sprite, Boolean rotateImage)
	{
		this.actionName = Constants.ACTION_MOVE_DOWN;
		this.sprite = sprite;
	...
	{
		if(this.rotateImage){
			if(gameObject.getAngle() == 270){
				gameObject.moveObject();
			}
			else{
				int rotate = 270 - gameObject.getAngle();
				gameObject.updateAngle(rotate);
				gameObject.updateImageRotationAngle(rotate);
				gameObject.moveObject();
			}
		}
		else gameObject.moveDown();
	}
