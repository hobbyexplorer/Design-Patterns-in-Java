...
(function($){
			...
			if(node === "a" && s.ajaxTagA){
				$(this).click(function(){
					...
					if(href){
						if(href.indexOf('#') >= 0){
							$.zxxbox($(href), options);
						}else if(/[\.jpg\.png\.bmp\.jpeg]$/i.test(href)){
							//加载图片
	...
	$.zxxbox = function(elements, options){
		...
		var s = $.extend({}, zxxboxDefault, options || {});
		//弹框的显示
		var WRAP = '<div id="zxxBlank" onselectstart="return false;"></div><div class="wrap_out" id="wrapOut"><div class="wrap_in" id="wrapIn"><div id="wrapBar" class="wrap_bar"  onselectstart="return false;"><div class="wrap_title"><span>'+s.title+'</span></div><a href="javasctipt:void(0);" class="wrap_close" id="wrapClose">'+s.shut+'</a></div><div class="wrap_body" id="wrapBody"></div></div></div>';
		if($("#wrapOut").size()){
			$("#wrapOut").show();
			if(s.bg){
				$("#zxxBlank").show();	
	...
	var zxxboxDefault = {
		...
		onshow: function(){ //弹窗显示后触发事件
			$.noop();
		},
		onclose: function(){ //弹窗关闭后触发事件
			$.noop();
		},
